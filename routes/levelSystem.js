...
export const getPointBreakdown = (userTricks, userArticleStatus = {}) => {
  ...
  if (userTricks && typeof userTricks === 'object') {
    Object.values(userTricks).forEach((trick) => {
      if (trick?.status === 'mastered' || trick?.regular_status === 'mastered') {
        trickPoints += POINT_VALUES.trick_mastered_regular;
        regularCount++;
      }
      if (trick?.goofy_status === 'mastered') {
        trickPoints += POINT_VALUES.trick_mastered_goofy;
        goofyCount++;
      }
    });
  }
  ...
  if (userArticleStatus && typeof userArticleStatus === 'object') {
    Object.values(userArticleStatus).forEach((status) => {
      if (status === 'known') {
        articlePoints += POINT_VALUES.article_read;
        articleCount++;
      }
    });
  }
...
export const countInProgressTricks = (userTricks) => {
  ...
  return Object.values(userTricks).filter(
    (t) =>
      ...
      t?.regular_status === 'in_progress' ||
      t?.goofy_status === 'in_progress'
  ).length;
};
...
export const projectLevel = (currentPoints, additionalTricks, bothSides = true) => {
  ...
  return {
    ...
    currentPoints,
    additionalTricks,
    pointsGain: additionalTricks * pointsPerTrick,
    projectedPoints,
    projectedLevel,
    levelUp: projectedLevel.level > currentLevel.level,
    newLevelName: projectedLevel.name,
  };
};
...
export const getLevelData = (userTricks, userArticleStatus = {}) => {
  ...
  const projection = inProgressCount > 0
    ? projectLevel(points, inProgressCount, true)
    : null;
  ...
  return {
    ...
    levelIcon: progress.current.icon,
    levelColor: progress.current.color,
