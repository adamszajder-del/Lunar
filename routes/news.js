...
router.get('/run-products-migration', async (req, res) => {
  ...
  try {
    ...
    try {
      await db.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS icon VARCHAR(50) DEFAULT NULL`);
      results.steps.push('✅ icon column added/verified');
    } catch (err) {
      results.steps.push(`⚠️ icon column: ${err.message}`);
    }

    // Add duration column if missing
    try {
      await db.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS duration VARCHAR(50) DEFAULT NULL`);
      results.steps.push('✅ duration column added/verified');
    } catch (err) {
      results.steps.push(`⚠️ duration column: ${err.message}`);
    }

    // Add gradient column if missing
    try {
      await db.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS gradient VARCHAR(255) DEFAULT NULL`);
      results.steps.push('✅ gradient column added/verified');
    } catch (err) {
      results.steps.push(`⚠️ gradient column: ${err.message}`);
    }

    // Add image_url column if missing
    try {
      await db.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS image_url TEXT DEFAULT NULL`);
      results.steps.push('✅ image_url column added/verified');
    } catch (err) {
      results.steps.push(`⚠️ image_url column: ${err.message}`);
    }

    // Add stripe_price_id column if missing
    try {
      await db.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS stripe_price_id VARCHAR(100) DEFAULT NULL`);
      results.steps.push('✅ stripe_price_id column added/verified');
    } catch (err) {
      results.steps.push(`⚠️ stripe_price_id column: ${err.message}`);
    }
  } catch (error) {
    results.success = false;
    results.errors.push(error.message);
