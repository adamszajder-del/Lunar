  ...
  try {
    ...
    if (order.stripe_session_id) {
      ...
      if (session.payment_status === 'paid') {
        const newStatus = order.product_category === 'clothes' ? STATUS.PENDING_SHIPMENT : STATUS.COMPLETED;
...
router.get('/my', authMiddleware, async (req, res) => {
  try {
    const result = await db.query(`
      ...
      FROM orders 
      WHERE user_id = $1 AND status NOT IN ('${STATUS.PENDING_PAYMENT}', '${STATUS.CANCELLED}')
  ...
  try {
    const result = await db.query(`
      ...
      WHERE user_id = $1 
        ...
        AND status IN ('${STATUS.COMPLETED}', '${STATUS.PENDING_SHIPMENT}')
