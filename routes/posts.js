...
router.delete('/:postId', validateId('postId'), authMiddleware, async (req, res) => {
  try {
    const result = await db.query(
...
// ─── LIKE / UNLIKE POST ───
router.post('/:postId/like', validateId('postId'), authMiddleware, async (req, res) => {
  try {
...
// ─── GET COMMENTS ───
router.get('/:postId/comments', validateId('postId'), authMiddleware, async (req, res) => {
  try {
    const postId = parseInt(req.params.postId);
    const userId = req.user.id;
...
// ─── ADD COMMENT ───
router.post('/:postId/comment', validateId('postId'), authMiddleware, async (req, res) => {
  try {
    const postId = parseInt(req.params.postId);
    const userId = req.user.id;
    const { content } = req.body;
...
router.delete('/:postId/comments/:commentId', validateId('postId', 'commentId'), authMiddleware, async (req, res) => {
  try {
    const result = await db.query(
...
// ─── LIKE COMMENT ───
router.post('/:postId/comments/:commentId/like', validateId('postId', 'commentId'), authMiddleware, async (req, res) => {
  try {
