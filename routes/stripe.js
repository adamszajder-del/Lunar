...
module.exports.handleWebhook = async (req, res) => {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      const orderResult = await db.query(
        `UPDATE orders SET status = CASE WHEN product_category = 'clothes' THEN '${STATUS.PENDING_SHIPMENT}' ELSE '${STATUS.COMPLETED}' END, stripe_payment_intent = $1 WHERE stripe_session_id = $2 AND status = '${STATUS.PENDING_PAYMENT}' RETURNING *`,
