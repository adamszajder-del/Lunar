  ...
  try {
    ...
    const result = await db.query(`
      ...
      LEFT JOIN (
        SELECT 
          user_id,
          (COUNT(*) FILTER (WHERE status = $3) + COUNT(*) FILTER (WHERE COALESCE(goofy_status, '${STATUS.TODO}') = $3)) as mastered,
          (COUNT(*) FILTER (WHERE status = $4) + COUNT(*) FILTER (WHERE COALESCE(goofy_status, '${STATUS.TODO}') = $4)) as in_progress
        FROM user_tricks
        GROUP BY user_id
...
router.get('/leaderboard', async (req, res) => {
  try {
    ...
    const result = await db.query(`
      ...
      FROM users u
      LEFT JOIN (
        SELECT user_id, COUNT(*) FILTER (WHERE status = '${STATUS.MASTERED}' ${tricksDateFilter}) + COUNT(*) FILTER (WHERE COALESCE(goofy_status,'${STATUS.TODO}') = '${STATUS.MASTERED}' ${tricksDateFilter}) as mastered
...
router.get('/:id/tricks', validateId('id'), async (req, res) => {
  try {
    ...
    const result = await db.query(`
      SELECT ut.id, ut.trick_id, ut.status, COALESCE(ut.goofy_status, '${STATUS.TODO}') as goofy_status, ut.updated_at,
...
router.get('/:id/tricks/reactions', validateId('id'), authMiddleware, async (req, res) => {
  try {
    ...
    const tricksResult = await db.query(
      `SELECT trick_id FROM user_tricks WHERE user_id = $1 AND (status = $2 OR COALESCE(goofy_status, '${STATUS.TODO}') = $2)`,
  ...
  try {
    ...
    const query = `
      WITH trick_feed AS (
        ...
        WHERE ut.user_id = $1 AND (ut.status IN ('${STATUS.MASTERED}', '${STATUS.IN_PROGRESS}') OR COALESCE(ut.goofy_status, '${STATUS.TODO}') IN ('${STATUS.MASTERED}', '${STATUS.IN_PROGRESS}'))
      ),
      event_feed AS (
        SELECT 
